...
def checkout_head_branch(
    ...
    if (
        ...
        dev_env.shell.run_or_raise(
            "git",
            ["switch", "-C", head_branch, "--track", f"{remote}/{head_branch}"],
            dev_env.root,
        )
...
def _get_head_branch(
    ...
    if len(dev_env_remotes) > 0:
        matching_remotes: List[GitRemote] = [
            remote
    ...
    if remote_name is not None:
        head_branch: Optional[str] = GetGitRemoteHeadBranch.run(
            dev_env.root, remote_name, dev_env.shell
        )
    ...
    remotes_response: ShellResponse = dev_env.shell.run_or_raise(
        "git", ["remote"], dev_env.root
    )
    remote_names: List[str] = [
        remote.strip() for remote in remotes_response.get_stdout_lines()
    ]
    ...
    for name in remote_names:
        head_branch: Optional[str] = GetGitRemoteHeadBranch.run(
            dev_env.root, name, dev_env.shell
        )
...
def main() -> None:
    ...
    checkout_head_branch(parsed_args.project, parsed_args.remote_name)
