...
class Shell:
    ...
    def run_or_raise(self,
                     ...
                     exception: Optional[Union[Exception, Callable[[ShellResponse], Exception]]] = None) \
            -> ShellResponse:
        ...
        if exception is None:
            raise Exception(f"The command '{response.command_line}' returned {response.exit_code}:\n"
                            f"stdout:\n'{response.stdout}'\n"
                            f"stderr:\n'{response.stderr}'")
    ...
    def run(self,
        ...
        process_arguments: List[str] = self._assemble_process_arguments(command, arguments)
        directory: Path = working_directory.resolve() if working_directory else Path(".").resolve()

        return self._execute(process_arguments, directory)

    def _assemble_process_arguments(self, command: str, arguments: Optional[List[str]] = None) -> List[str]:
