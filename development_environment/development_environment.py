...
class DevelopmentEnvironment:
    ...
    @staticmethod
    def load(
        current_directory: Path, shell: Optional[Shell] = None
    ) -> "DevelopmentEnvironment":
        root_dir: Optional[Path] = DevelopmentEnvironment._get_root_directory(
            current_directory
        )

        git_configuration: GitConfiguration = GitConfiguration.load(root_dir)
        formatter_configuration: FormatterConfiguration = FormatterConfiguration.load(
            root_dir
        )
    ...
    def _get_root_directory(current_directory: Path) -> Optional[Path]:
        while current_directory != current_directory.parent:
            root_dir: Path = Path(
                current_directory, DevelopmentEnvironment.ROOT_DIRECTORY
            )
            ...
            git_dir: Path = Path(
                current_directory, DevelopmentEnvironment.GIT_DIRECTORY
            )
            if git_dir.is_dir():
