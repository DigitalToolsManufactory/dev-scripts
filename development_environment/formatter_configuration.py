...
from typing import Any, Dict, List, Union, Optional, ClassVar, Callable
...
class FormatterConfiguration(ABC):
    ...
    def from_json(content: Union[str, Dict[str, Any]]) -> "FormatterConfiguration":
        ...
        if nameof(FormatterConfiguration.formatter_type) not in data:
            raise AssertionError(f"The required property '{nameof(FormatterConfiguration.formatter_type)}' "
    ...
    # region infer
    @staticmethod
    def infer(directory: Path, shell: Optional[Shell] = None) -> "FormatterConfiguration":
        infer_functions: Dict[str, Callable[[Path, Optional[Shell]], "FormatterConfiguration"]] = {
...
class MavenFormatterConfiguration(FormatterConfiguration):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result[nameof(MavenFormatterConfiguration.excluded_modules)] = self.excluded_modules
        result[nameof(MavenFormatterConfiguration.additional_arguments)] = self._additional_arguments
    ...
    def from_dict(data: Dict[str, Any]) -> "MavenFormatterConfiguration":
        ...
        if nameof(MavenFormatterConfiguration.excluded_modules) in data:
            excluded_modules.extend(list(data[nameof(MavenFormatterConfiguration.excluded_modules)]))

        additional_arguments: List[str] = []
        if nameof(MavenFormatterConfiguration.additional_arguments) in data:
            additional_arguments.extend(list(data[nameof(MavenFormatterConfiguration.additional_arguments)]))

        return MavenFormatterConfiguration(goals, excluded_modules, additional_arguments)

    @staticmethod
    def infer(directory: Path, shell: Optional[Shell] = None) -> "MavenFormatterConfiguration":
