...
class MockShellInvocation:
    ...
    def get_command_line(self) -> str:
        return self.command + f" {self.arguments}" if self.arguments is not None and len(self.arguments) > 0 else ""
...
class MockShell(Shell):
    ...
    def run(self,
            ...
            working_directory: Optional[Path] = None) -> ShellResponse:
        invocation: MockShellInvocation = MockShellInvocation(command,
                                                              self._combine_arguments(arguments),
                                                              working_directory)
        for rule in self._mock_rules:
            if rule.matches(invocation):
                return rule.get_shell_response(invocation.get_command_line())

        raise AssertionError(f"No rule specified that matches following command line: {invocation.get_command_line()}")

    def _combine_arguments(self, arguments: List[str]) -> str:
        return " ".join(f"\"{argument.strip()}\"" if " " in argument else argument.strip()
                        for argument in arguments
                        if len(argument.strip()) > 0)

    # region MockRule builder
    def when_command(self, command_matcher: StringMatcher) -> "MockShell.InitialArgumentMatcherBuilder":
        return MockShell.InitialArgumentMatcherBuilder(self, command_matcher)

    class InitialArgumentMatcherBuilder:
        def __init__(self, mock_shell: "MockShell", command_matcher: Optional[StringMatcher]):
            self._mock_shell: "MockShell" = mock_shell
            self._command_matcher: Optional[StringMatcher] = command_matcher

        def has_argument(self, argument_matcher: StringMatcher) -> "MockShell.RepeatedArgumentMatcherBuilder":
            return MockShell.RepeatedArgumentMatcherBuilder(self._mock_shell, argument_matcher, self._command_matcher)

        def then_return(self, response: MockShellResponse) -> None:
            rule: MockShellRule = MockShellRule(MockShellInvocationMatcher(self._command_matcher,
                                                                           None),
                                                response)
